...
- Project creation with normalized slug, owner membership, platform hostname, runner tag, and lifecycle state.
- GitLab identity linkage plus project repository provisioning interface.
- Kubernetes namespace provisioning interface and base tenant manifests for namespace, quota, RBAC, network policy, app/static/frontend services, ingress, HTTPRoute, certificates, and external secrets.
...
3. Backend normalizes the slug, creates project metadata, owner membership, default platform hostname, namespace name, runner tag, and lifecycle plan.
4. User lands on the project overview and sees status, GitLab path, namespace, platform hostname, domains, deployments, and environment variables.
5. User can invite members, create API tokens, configure environment variables, and continue to provisioning.

### GitLab/Kubernetes project deploy

1. User links a GitLab identity if not already linked.
2. User starts repository provisioning from dashboard or API.
3. Backend calls the GitLab service and records repository URL/path and runner configuration status.
4. User starts Kubernetes namespace provisioning.
5. Backend calls the Kubernetes service and marks namespace, quota, RBAC, and policies ready.
6. User triggers a deployment for a Git ref or commit SHA.
7. Backend creates a queued/running deployment, receives status reports from CI/runner integration, appends logs, and updates lifecycle state.
8. User monitors deployment status and logs; if needed, user rolls back to a previous deployment.
...
| Kubernetes namespace provisioning | Backend path exists; base manifests define tenant namespace and related resources. | `apps/backend/src/backend-service.ts`, `infra/k8s/base` |
| Deployment tracking/logs/rollback | Backend routes and dashboard client types/methods exist for deployments, reports, logs, and rollback. | `apps/backend/src/backend-service.ts`, `apps/frontend/src/dashboard.ts` |
...
| Kubernetes apply integration | Infrastructure | P0 | Templates exist | Cluster target, kube credentials, manifest rendering | Namespace, quota, RBAC, network policy, services, and routing are rendered and applied for a project. |
...
- [ ] Kubernetes namespace provisioning applies manifests to the selected cluster.
...
- [ ] End-to-end tests and synthetic checks continuously cover signup, project deploy, domain attach, TLS, deployment rollback, and instant publish if launched.
